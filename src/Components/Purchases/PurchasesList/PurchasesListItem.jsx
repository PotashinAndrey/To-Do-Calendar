...
export default function PurchasesListItem({ purchaseNote }) {
  ...
  return (
    <div className={"PurchasesListItemWrapper " + purchaseNote.priority}>
      ...
      { purchaseNote.discription?.length ? <Text type="secondary">{ purchaseNote.discription.length > 16 ? purchaseNote.discription.slice(0, 15) + '...' : purchaseNote.discription.length}</Text> : <Text type="secondary">Нет описания</Text> }
